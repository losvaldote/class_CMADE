...
int background_functions(
  ...
  if (pba->has_lambda == _TRUE_) {
    ...
    /* CMaDE */
    /*if(a_rel < exp(-12.0) ){
      pvecback[pba->index_bg_rho_lambda] = (0.1888/(3.0*pow(pba->H0,2)))*pow(pba->H0,2);
    }
    else if(a_rel>=exp(-12.0) && a_rel<exp(-3.0)){
      pvecback[pba->index_bg_rho_lambda] = (pow(0.1833 + 1.0/pow(log(a_rel)-1.404832506,2),1.01)/(3.0*pow(pba->H0,2)))*pow(pba->H0,2);
    }
    else if(a_rel>=exp(-3.0) && a_rel<exp(-0.5)){
      pvecback[pba->index_bg_rho_lambda] = (pow(0.1833 + 1.0/pow(log(a_rel)-1.404832506,2),0.85 - 0.055*log(a_rel))/(3.0*pow(pba->H0,2)))*pow(pba->H0,2);
    }
    //else if(a_rel>=exp(-0.5)){
    else {
      pvecback[pba->index_bg_rho_lambda] = (pow(0.1833 + 1.0/pow(log(a_rel)-1.404832506,2),0.9)/(3.0*pow(pba->H0,2)))*pow(pba->H0,2);
    }*/
    /* CMaDE  prueba 2*/
    /*if(a_rel < exp(-12.0) ){
      pvecback[pba->index_bg_rho_lambda] = (0.1888)*(pow(10,6)/(3*pow(_c_,2)));
    }
    else if(a_rel>=exp(-12.0) && a_rel<exp(-3.0)){
      pvecback[pba->index_bg_rho_lambda] = pow(0.1833 + 1.0/pow(log(a_rel)-1.404832506,2),1.01)*(pow(10,6)/(3*pow(_c_,2)));
    }
    else if(a_rel>=exp(-3.0) && a_rel<exp(-0.5)){
      pvecback[pba->index_bg_rho_lambda] = pow(0.1833 + 1.0/pow(log(a_rel)-1.404832506,2),0.85 - 0.055*log(a_rel))*(pow(10,6)/(3*pow(_c_,2)));
    }
    //else if(a_rel>=exp(-0.5)){
    else {
      pvecback[pba->index_bg_rho_lambda] = pow(0.1833 + 1.0/pow(log(a_rel)-1.404832506,2),0.9)*(pow(10,6)/(3*pow(_c_,2)));
    }*/
    /* CMaDE  prueba 3*/
    /*if(a_rel < exp(-12.0) ){
    ...
    //else if(a_rel>=exp(-0.5)){
      ...
      pvecback[pba->index_bg_rho_lambda] = pow(0.1833 + 1.0/pow(log(a_rel)-1.404832506,2),0.9)*pow(pba->H0,2);
    }*/
    /*       */
    /* CMaDE  erf approximation*/
    /* 22/02/2022*/
  ...
  //if (pba->background_verbose > 0) {
    ...
    if (pba->has_sfdm_1 == _TRUE_){
        ...
        /*Print cosh mass information*/
          ...
          //29-04-21
          printf("-> Masa prueba 1 = %1.2e [eV]\n",
                      //Eq 2.8
                      3.19696e-30*1.5*pba->H0*pow(pba->sfdm_parameters_1[1]/3.-4.,2.)*pow(pba->Omega0_sfdm_1/(pba->Omega0_g+pba->Omega0_ur + pba->Omega0_ncdm_tot),2.));
                      //Eq 2.7
                      //1.0e-22*2.57e-10*(pba->H0/100.0)*pow(pba->sfdm_parameters_1[1]/12.0,2.0)*pow(1.0-12.0/pba->sfdm_parameters_1[1],3./2.)*pow(pba->Omega0_sfdm_1,2.)/pow(pba->Omega0_g+pba->Omega0_ur,3./2.));
          printf(" -> Radiacion total = %1.2e\n", pba->Omega0_g+pba->Omega0_ur + pba->Omega0_ncdm_tot);
          printf(" -> Prueba H0 = %1.2e\n",
                      pba->H0);
        }
