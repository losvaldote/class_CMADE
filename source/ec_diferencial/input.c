...
int input_init(
  ...
  char * const target_namestrings[] = {"100*theta_s","Omega_dcdmdr","omega_dcdmdr","Omega_sfdm_1", "Omega_sfdm_2",
                                       "Omega_scf","Omega_ini_dcdm","omega_ini_dcdm","sigma8",/*CMADE*/ "H0"/*"CMADE_aux"*/};
  char * const unknown_namestrings[] = {"h","Omega_ini_dcdm","Omega_ini_dcdm","sfdm_shooting_parameter_1", "sfdm_shooting_parameter_2",
                                        "scf_shooting_parameter","Omega_dcdmdr","omega_dcdmdr","A_s", /*CMADE*/"H_shooting_parameter"};
...
int input_read_parameters(
  ...
  /*Shooting parameter for CMADE Hubble*/
  class_call(parser_read_double(pfc,"CMADE_aux",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  pba->CMADE_aux = param1;
  class_read_double("H_shooting_parameter", pba->CMADE_aux);
  pba->H_ini = pba->H0*9.9999e25 + pba->CMADE_aux;
  //pba->H_ini = pba->H_aux;
...
int input_try_unknown_parameters(double * unknown_parameter,
  ...
  for (i=0; i < pfzw->target_size; i++) {
    ...
    case sigma8:
      ...
      break;
    case H0:
      output[i] = ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_H]
        -ba.H0;
      break;
    }
  }
...
int input_get_guess(double *xguess,
  ...
  * - Version 3: use attractor solution */
    ...
    case H0:
    /* Default: take the passed value as xguess and set dxdy to 1. */
    //xguess[index_guess] = ba.H0*9.9999e17 + ba.H_aux;
    xguess[index_guess] = ba.CMADE_aux;
    dxdy[index_guess] = 1.;
    break;
    }
    //printf("xguess = %g\n",xguess[index_guess]);
