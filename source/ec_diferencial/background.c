...
int background_functions(
  ...
  //CMADE
  //Aproximacion ec 11
  //pvecback[pba->index_bg_H] = sqrt(rho_tot-pvecback[pba->index_bg_rho_lambda] + pvecback[pba->index_bg_rho_lambda]/a -pba->K/a/a);
  //Solucion de ec 8
  H_aux = pvecback_B[pba->index_bi_H];
  pvecback[pba->index_bg_H] = H_aux;
  /** - compute derivative of H with respect to conformal time */
  //pvecback[pba->index_bg_H_prime] = - (3./2.) * (rho_tot + p_tot) * a + pba->K/a;
  //CMADE
  //Aproximacion ec 11
  //pvecback[pba->index_bg_H_prime] = - (3./2.) * (rho_tot-pvecback[pba->index_bg_rho_lambda]+pvecback[pba->index_bg_rho_lambda]/a + p_tot
  //                                               +pvecback[pba->index_bg_rho_lambda]-pvecback[pba->index_bg_rho_lambda]/a) * a + pba->K/a;
  pvecback[pba->index_bg_H_prime] = a*H_aux*(-sqrt(3./2.)*(pow(pba->H0,3)/pow(H_aux,2))
                        *(1./_PI_*a)*
                        (pow(pow(H_aux/pba->H0,2)-(pba->Omega0_b+pba->Omega0_cdm)/pow(a,3.)
                        -(pba->Omega0_ur+pba->Omega0_g)/pow(a,4.),3./2.))
                        -3./2.*pow(pba->H0,2)*(pba->Omega0_b+pba->Omega0_cdm)/H_aux/pow(a,3.)
                        -2.*pow(pba->H0,2)*(pba->Omega0_ur+pba->Omega0_g)/H_aux/pow(a,4.));
...
int background_indices(
  ...
  /* -> Hubble parameter for CMADE*/
  class_define_index(pba->index_bi_H,_TRUE_,index_bi,1);
...
int background_solve(
  ...
  while (pvecback_integration[pba->index_bi_a] < pba->a_today) {
    ...
    if ((pvecback_integration[pba->index_bi_a]*(1.+ppr->back_integration_stepsize)) < pba->a_today) {
      ...
      //CMADE
      tau_end = tau_start + ppr->back_integration_stepsize / (pvecback_integration[pba->index_bi_a]*pvecback_integration[pba->index_bi_H]);
    }
    else {
      ...
      /* no possible segmentation fault here: non-zeroness of "a" has been checked in background_functions() */
      //CMADE
      tau_end = tau_start + (pba->a_today/pvecback_integration[pba->index_bi_a]-1.) / (pvecback_integration[pba->index_bi_a]*pvecback_integration[pba->index_bi_H]);
    }
...
int background_initial_conditions(
  ...
  */
  ...
  /*CMADE Hubble parameter initial condition*/
  //pvecback_integration[pba->index_bi_H] = pba->H0*3.2477e17;
  //pvecback_integration[pba->index_bi_H] = 2.157545689084e+22;
  /*Prueba shooting*/
  pvecback_integration[pba->index_bi_H] = pba->H_ini;
...
int background_derivs(
  ...
  dy[pba->index_bi_a] = y[pba->index_bi_a] * y[pba->index_bi_a] * y[pba->index_bi_H];

  /* - Calculate CMADE Hubble parameter*/
  dy[pba->index_bi_H] = y[pba->index_bi_a]*y[pba->index_bi_H]*(-sqrt(3./2.)*(pow(pba->H0,3)/pow(y[pba->index_bi_H],2))
                        *(1./_PI_*y[pba->index_bi_a])*
                        (pow(pow(y[pba->index_bi_H]/pba->H0,2)-(pba->Omega0_b+pba->Omega0_cdm)/pow(y[pba->index_bi_a],3.)
                        -(pba->Omega0_ur+pba->Omega0_g)/pow(y[pba->index_bi_a],4.),3./2.))
                        -3./2.*pow(pba->H0,2)*(pba->Omega0_b+pba->Omega0_cdm)/y[pba->index_bi_H]/pow(y[pba->index_bi_a],3.)
                        -2.*pow(pba->H0,2)*(pba->Omega0_ur+pba->Omega0_g)/y[pba->index_bi_H]/pow(y[pba->index_bi_a],4.));
  ...
  if (pba->has_dcdm == _TRUE_){
    ...
    dy[pba->index_bi_rho_dcdm] = -3.*y[pba->index_bi_a]*y[pba->index_bi_H]*y[pba->index_bi_rho_dcdm]-
      y[pba->index_bi_a]*pba->Gamma_dcdm*y[pba->index_bi_rho_dcdm];
  }

  if ((pba->has_dcdm == _TRUE_) && (pba->has_dr == _TRUE_)){
    /** - Compute dr density \f$ \rho' = -4aH \rho - a \Gamma \rho \f$ */
    dy[pba->index_bi_rho_dr] = -4.*y[pba->index_bi_a]*y[pba->index_bi_H]*y[pba->index_bi_rho_dr]+
      y[pba->index_bi_a]*pba->Gamma_dcdm*y[pba->index_bi_rho_dcdm];
  }

  if (pba->has_fld == _TRUE_) {
    /** - Compute fld density \f$ \rho' = -3aH (1+w_{fld}(a)) \rho \f$ */
    dy[pba->index_bi_rho_fld] = -3.*y[pba->index_bi_a]*y[pba->index_bi_H]*(1.+pvecback[pba->index_bg_w_fld])*y[pba->index_bi_rho_fld];
  }
  ...
  if (pba->has_sfdm_1 == _TRUE_){
    ...
    /*Quadratic and cos*/
    if (pba->sfdm_parameters_1[1] >= 0.){
      dy[pba->index_bi_alpha_sfdm_1] = 3.*y[pba->index_bi_a]*y[pba->index_bi_H]*
        (pvecback[pba->index_bg_w_tot_1]+cos_sfdm(pba,y[pba->index_bi_theta_sfdm_1]));
      dy[pba->index_bi_theta_sfdm_1] = y[pba->index_bi_a]*y[pba->index_bi_H]*
        (-3.*sin_sfdm(pba,y[pba->index_bi_theta_sfdm_1])+y[pba->index_bi_y1_sfdm_1]);
      dy[pba->index_bi_y1_sfdm_1] = y[pba->index_bi_a]*y[pba->index_bi_H]*
    ...
    /*Cosh*/
     ...
     //General expression for: axion (lambda >0), quadratic (lambda =0), cosh (lambda < 0)
      dy[pba->index_bi_y1_sfdm_1] = 1.5*y[pba->index_bi_a]*y[pba->index_bi_H]*
  ...
  if (pba->has_sfdm_2 == _TRUE_){
    ...
    /*Quadratic and cos*/
    if (pba->sfdm_parameters_2[1] >= 0.){
    dy[pba->index_bi_alpha_sfdm_2] = 3.*y[pba->index_bi_a]*y[pba->index_bi_H]*
      (pvecback[pba->index_bg_w_tot_2]+cos_sfdm(pba,y[pba->index_bi_theta_sfdm_2]));
    dy[pba->index_bi_theta_sfdm_2] = y[pba->index_bi_a]*y[pba->index_bi_H]*
      (-3.*sin_sfdm(pba,y[pba->index_bi_theta_sfdm_2])+y[pba->index_bi_y1_sfdm_2]);
    dy[pba->index_bi_y1_sfdm_2] = y[pba->index_bi_a]*y[pba->index_bi_H]*
      ...
      dy[pba->index_bi_theta_sfdm_2] = y[pba->index_bi_a]*y[pba->index_bi_H]*
        ...
        pow(1.-pba->sfdm_parameters_2[1]*exp(y[pba->index_bi_alpha_sfdm_2]-2.*y[pba->index_bi_y1_sfdm_2])
           *(1.+cos_sfdm(pba,y[pba->index_bi_theta_sfdm_2])),0.5));
              //pow(pow(y[pba->index_bi_y_phi_scf],2.)-pba->scf_parameters[0]*exp(y[pba->index_bi_Omega_phi_scf])*(1.+cos_scf(pba,y[pba->index_bi_theta_phi_scf])),0.5));
     //General expression for: axion (lambda >0), quadratic (lambda =0), cosh (lambda < 0)
      dy[pba->index_bi_y1_sfdm_2] = 1.5*y[pba->index_bi_a]*y[pba->index_bi_H]*
  ...
  if (pba->has_scf == _TRUE_){
    ...
    dy[pba->index_bi_phi_prime_scf] = - y[pba->index_bi_a]*
      (2*y[pba->index_bi_H]*y[pba->index_bi_phi_prime_scf]
